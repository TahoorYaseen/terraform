...
func (d *evaluationStateData) staticValidateReference(ref *addrs.Reference, self, source addrs.Referenceable) tfdiags.Diagnostics {
...
func (d *evaluationStateData) staticValidateResourceReference(modCfg *configs.Config, addr addrs.Resource, source addrs.Referenceable, remain hcl.Traversal, rng tfdiags.SourceRange) tfdiags.Diagnostics {
	...
	if cfg.Check != nil && (source == nil || source != cfg.Check.Addr()) {
		diags = diags.Append(&hcl.Diagnostic{
			Severity: hcl.DiagError,
			Summary:  `Reference to hidden resource`,
			Detail:   fmt.Sprintf(`The %s resource %q %q has been declared in a check block, and is not available from %s.`, modeAdjective, addr.Type, addr.Name, moduleConfigDisplayAddr(modCfg.Path)),
			Subject:  rng.ToHCL().Ptr(),
		})
	}
